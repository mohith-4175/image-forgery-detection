...
# PATHS
# --------------------------------------------------
ONNX_MODEL_PATH = os.path.join(BASE_DIR, "ml", "models", "cnn_model.onnx")
...
def generate_mock_heatmap(img_array):
    """Generate heatmap using edge detection (no TF)"""
...
def process_image(image_path):
    ...
    # ONNX prediction
    outputs = session.run(None, {"input": cnn_input})
    cnn_prob = float(outputs[0][0][0])
